...
static const char asciiTable[94][2] PROGMEM = {
  { "!" },
  { "\"" },
  { "#" },
  { "$" },
...
static const byte asciiWidth[94] = {
  1,  // "!"
  3,  // """
  5,  // "#"
  5,  // "$"
...
#define kRamSize (TINY_BASIC_MEMORY - 1160 - kRamFileIO - kRamTones)
  ...
  switch (inStream) {
    ...
    default:
      while (1) {
        ...
        if (vga.millis() - lastBlinkedTime >= CURSOR_BLINK_PERIOD) {
          blink = !blink;
          lastBlinkedTime = vga.millis();
        }
        vgaPrintAscii(95, x, y, blink ? 1 : 0);
      }
  }
...
void vgaPrintAscii(byte c, byte x, byte y, byte color) {
  vga.printPROGMEM((byte *)fnt_nanofont_data, FNT_NANOFONT_SYMBOLS_COUNT, FNT_NANOFONT_HEIGHT, 0, 0, asciiTable[c - 33], x, y, color);
  vga.delay(1);
}
